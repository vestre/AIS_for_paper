...
def time_to_CPA_calculator(whole_table, 
    ...
    outmatrix = _CPA_calculator_workhorse(
        ...
        np.asarray(whole_table[namedict["Length"]]),
        np.asarray(Indices),
    ...
    LONs = np.asarray(whole_table[namedict["LON"]])
    LATs = np.asarray(whole_table[namedict["LAT"]])
    Lengths = np.asarray(whole_table[namedict["Length"]])
    ...
    LON_V1 = LONs[zeros_mask]
    LAT_V1 = LATs[zeros_mask]
    Length_V1 = Lengths[zeros_mask]
    ...
    LON_V2_tot = LONs[indices_mins]
    LAT_V2_tot = LATs[indices_mins]
    Length_V2_tot = Lengths[indices_mins]
    ...
    LON_V2 = LON_V2_tot[zeros_mask]
    LAT_V2 = LAT_V2_tot[zeros_mask]
    Length_V2 = Length_V2_tot[zeros_mask]
...
@numba.jit(parallel=True, nopython=True, fastmath=True)
def _CPA_calculator_workhorse(IDs, LATs, LONs, DateTimes,
                            SOGs, COGs, Intervals, Lengths, Indices,
                            extrainterval=False):
    """
...
def observation_collector(bigtable, selectortable,
    ...
    # as standard in function standard call)
...
def statistics_aggregator(whole_table, 
    ...
    (sitnum, recnum, t1_inds ,tF_inds ,t1s, tFs, tMs,
            delTimes, LAT1_t1, LON1_t1, LAT2_t1, LON2_t1,
            #meanSOG1, meanSOG2, meanCOG1, meanCOG2,
            deltaCOG,
            COLREGs, dist_yields1, dist_yields2, pass_dist, 
            vesseldist_yield, app_speed, 
            tot_SOG1_change, tot_SOG2_change, tot_COG1_change, tot_COG2_change,
            steady_SOG1_change, steady_SOG2_change, 
            steady_COG1_change, steady_COG2_change,
            #max_dSOG1, max_dSOG2, max_dCOG1, max_dCOG2, 
            COG_maneuver1, COG_maneuver2, SOG_maneuver1, SOG_maneuver2, 
            COG_overshoot1, COG_overshoot2, SOG_overshoot1, SOG_overshoot2,
            COG_alt_maneuver1, COG_alt_maneuver2,
        ...
        np.asarray(whole_table[COGcolname1]),
        np.asarray(whole_table[COGcolname2]),
        np.asarray(whole_table[LENcolname1]),
        np.asarray(whole_table[LENcolname2]),
        np.asarray(whole_table[sitcolname]),
    ...
    outtable = pd.DataFrame({
            ...
            'Vessel_distance_at_yield': vesseldist_yield,
            ...
            'Steady_state_speed_change_1': steady_SOG1_change, 
            'Steady_state_speed_change_2': steady_SOG2_change, 
            'Steady_state_course_change_1': steady_COG1_change, 
            'Steady_state_course_change_2': steady_COG2_change,
            ...
            'Coursechange_1_overshoot': COG_overshoot1, 
            'Coursechange_2_overshoot': COG_overshoot2, 
            'Speedchange_1_overshoot': SOG_overshoot1, 
            'Speedchange_2_overshoot': SOG_overshoot2,
            'Course_maneuver_alt_1': COG_alt_maneuver1, 
            'Course_maneuver_alt_2': COG_alt_maneuver2,
            'Speed_maneuver_alt_1': SOG_alt_maneuver1, 
            'Speed_maneuver_alt_2': SOG_alt_maneuver2,
            'Coursechange_1_overshoot_alt': COG_alt_overshoot1, 
            ...
            columns=['Situation','Records',
                    ...
                    'Crossing_angle', 
                    'LAT_1_at_t1', 'LON_1_at_t1',
                    'LAT_2_at_t1', 'LON_2_at_t1',
                    'COLREG', 
                    'Distance_at_yield_1', 
                    'Distance_at_yield_2',
                    'Vessel_distance_at_yield', 
                    'Passing_distance', 
                    'Approach_speed',  
                    'Max_speed_change_1', 
                    'Max_speed_change_2', 
                    'Max_course_change_1', 
                    'Max_course_change_2', 
                    'Steady_state_speed_change_1',
                    'Steady_state_speed_change_2',
                    'Steady_state_course_change_1',
                    'Steady_state_course_change_2',
                    #'Max_dSOG_1', 'Max_dSOG_2', 
                    #'Max_dCOG_1', 'Max_dCOG_2',  
                    'Course_maneuver_1', 'Course_maneuver_2',
                    'Speed_maneuver_1','Speed_maneuver_2',
                    'Coursechange_1_overshoot', 'Coursechange_2_overshoot',
                    'Speedchange_1_overshoot', 'Speedchange_2_overshoot',
                    'Course_maneuver_alt_1','Course_maneuver_alt_2',
                    'Speed_maneuver_alt_1', 'Speed_maneuver_alt_2',
                    'Coursechange_1_overshoot_alt', 'Coursechange_2_overshoot_alt',
                    'Speedchange_1_overshoot_alt', 'Speedchange_2_overshoot_alt'
                    ] 
            )
    ...
    outtable["Situation"] = outtable.index + 1

    return outtable

def _aggregator_workhorse(ID1s, ID2s, LAT1s, LAT2s, LON1s, LON2s, Mergetimes, 
        SOG1s, SOG2s, COG1s, COG2s, Length1s, Length2s, Situations, 
        dist2CPA1s, dist2CPA2s, dSOG1s, dSOG2s, dCOG1s, dCOG2s, 
        tCPAs, CPAdists, Category1, Category2):
    """
    ...
    LON1_t1_collector = np.zeros(number_of_sits,dtype = np.float64)
    LAT1_t1_collector = np.zeros(number_of_sits,dtype = np.float64)
    LAT2_t1_collector = np.zeros(number_of_sits,dtype = np.float64)
    LON2_t1_collector = np.zeros(number_of_sits,dtype = np.float64)
    ...
    vesseldist_at_yield_collector = np.zeros(number_of_sits,dtype = np.float64)
    ...
    steady_SOG1_change_collector = np.zeros(number_of_sits,dtype = np.float64)
    steady_SOG2_change_collector = np.zeros(number_of_sits,dtype = np.float64)
    steady_COG1_change_collector = np.zeros(number_of_sits,dtype = np.float64)
    steady_COG2_change_collector = np.zeros(number_of_sits,dtype = np.float64)
    ...
    COG_overshoot1_collector = np.zeros(number_of_sits,dtype = np.float64)
    COG_overshoot2_collector = np.zeros(number_of_sits,dtype = np.float64)
    SOG_overshoot1_collector = np.zeros(number_of_sits,dtype = np.float64)
    SOG_overshoot2_collector = np.zeros(number_of_sits,dtype = np.float64)

    COG_alt_overshoot1_collector = np.zeros(number_of_sits,dtype = np.float64)
    COG_alt_overshoot2_collector = np.zeros(number_of_sits,dtype = np.float64)
    SOG_alt_overshoot1_collector = np.zeros(number_of_sits,dtype = np.float64)
    SOG_alt_overshoot2_collector = np.zeros(number_of_sits,dtype = np.float64)
    ...
    COG_alt_maneuver1_collector = np.zeros(number_of_sits,dtype = np.bool_)
    COG_alt_maneuver2_collector = np.zeros(number_of_sits,dtype = np.bool_)
    SOG_alt_maneuver1_collector = np.zeros(number_of_sits,dtype = np.bool_)
    SOG_alt_maneuver2_collector = np.zeros(number_of_sits,dtype = np.bool_)
        ...
        if records_num == 0:
            ...
            # of an absolute filter on meters (i.e. 400 m)
                ...
                if (np.sum(np.logical_and(Mergetime_sit >= t1 , 
                    ...
                    LAT1_t1_collector[situation] = (
                                        LAT1s[situation_filter][t1_ind])
                    LON1_t1_collector[situation] = (
                                        LON1s[situation_filter][t1_ind])
                    LAT2_t1_collector[situation] = (
                                        LAT2s[situation_filter][t1_ind])
                    LON2_t1_collector[situation] = (
                                        LON2s[situation_filter][t1_ind])

                    # Storing indices for possible robustness analysis 
                    # of final data
                    ...
                    Coords_Vessel_1 = (LAT1s[situation_filter][t1_ind], 
                                        LON1s[situation_filter][t1_ind])
                    Coords_Vessel_2 = (LAT2s[situation_filter][t1_ind], 
                                        LON2s[situation_filter][t1_ind])
                    ...
                    mean_COG1_firsthalf = st.circmean(COG1s_sit[
                        np.logical_and(Mergetime_sit >= t1 , 
                                        Mergetime_sit <= tM)],
                                                        high = 360,
                                                        low = 0)
                    
                    mean_COG2_firsthalf = st.circmean(COG2s_sit[
                        np.logical_and(Mergetime_sit >= t1 , 
                                        Mergetime_sit <= tM)],
                                                        high = 360,
                                                        low = 0)
                    ...
                    mean_COG1_pre = st.circmean(COG1s_sit[(Mergetime_sit <= t1)
                                                &(Mergetime_sit > (t1 - 20))],
                                                        high = 360,
                                                        low = 0)

                    mean_COG2_pre = st.circmean(COG2s_sit[(Mergetime_sit <= t1)
                                                &(Mergetime_sit > (t1 - 20))],
                                                        high = 360,
                                                        low = 0)

                    mean_COG1_post = st.circmean(COG1s_sit[(Mergetime_sit >= tF)
                                                &(Mergetime_sit < (tF + 20))],
                                                        high = 360,
                                                        low = 0)

                    mean_COG2_post = st.circmean(COG2s_sit[(Mergetime_sit >= tF)
                                                &(Mergetime_sit < (tF + 20))],
                                                        high = 360,
                                                        low = 0)


                    # Finding the mean SOG from before entering into the
                    # yielding maneuver for both vessels
                    mean_SOG1_pre = np.mean(SOG1s_sit[(Mergetime_sit <= t1)
                                                &(Mergetime_sit > (t1 - 20))])

                    mean_SOG2_pre = np.mean(SOG2s_sit[(Mergetime_sit <= t1)
                                                &(Mergetime_sit > (t1 - 20))])

                    mean_SOG1_post = np.mean(SOG1s_sit[(Mergetime_sit >= tF)
                                                &(Mergetime_sit < (tF + 20))])

                    mean_SOG2_post = np.mean(SOG2s_sit[(Mergetime_sit >= tF)
                                                &(Mergetime_sit > (t1 + 20))])

                    # Finding crossing angles by using the collected mean
                    # COG and SOGs and collecting this
                    delta_COG_pre = (mean_COG1_pre - mean_COG2_pre) % 360
                    delta_COG_collector[situation] = delta_COG_pre
                    ...
                    if np.cos(delta_COG_pre*np.pi/180) > np.cos(45*np.pi/180):
                        COLREG_collector[situation] = 1 

                    elif np.cos(delta_COG_pre*np.pi/180)< - np.cos(15*np.pi/180):
                    ...
                    dCOG1s_sit = np.abs(dCOG1s[situation_filter])
                    dCOG2s_sit = np.abs(dCOG2s[situation_filter])
                    dSOG1s_sit = np.abs(dSOG1s[situation_filter])
                    dSOG2s_sit = np.abs(dSOG2s[situation_filter])

                    # Finding largest differentiated COG and SOG before yield 
                    # maneuver
                    max_dCOG1_pre = np.max(dCOG1s_sit[range(max(0,t1_ind-60),t1_ind)])
                    max_dCOG2_pre = np.max(dCOG2s_sit[range(max(0,t1_ind-60),t1_ind)])
                    max_dSOG1_pre = np.max(dSOG1s_sit[range(max(0,t1_ind-60),t1_ind)])
                    max_dSOG2_pre = np.max(dSOG2s_sit[range(max(0,t1_ind-60),t1_ind)])
                    ...
                    if max_dCOG1_pre > TOL:
                        COG_overshoot1_collector[situation] = np.max(
                                            dCOG1s_int / max_dCOG1_pre)
                    else:
                        COG_overshoot1_collector[situation] = 0

                    if max_dCOG2_pre > TOL:
                        COG_overshoot2_collector[situation] = np.max(
                                            dCOG2s_int / max_dCOG2_pre)
                    else:
                        COG_overshoot2_collector[situation] = 0

                    if max_dSOG1_pre > TOL:
                        SOG_overshoot1_collector[situation] = np.max(
                                            dSOG1s_int / max_dSOG1_pre)
                    else:
                        SOG_overshoot1_collector[situation] = 0

                    if max_dSOG2_pre > TOL:
                        SOG_overshoot2_collector[situation] = np.max(
                                            dSOG2s_int / max_dSOG2_pre)
                    else:
                        SOG_overshoot2_collector[situation] = 0
                    ...
                    if Mergetime_sit[range(max(0,t1_ind-60),t1_ind)].size > 10:
                        ...
                        if mean_dCOG1_pre > TOL:
                            ...
                            COG_alt_overshoot1_collector[situation] = 0

                        if mean_dCOG2_pre > TOL:
                            COG_alt_overshoot2_collector[situation] = np.max(
                                                dCOG2s_int / mean_dCOG2_pre)
                        else:
                            COG_alt_overshoot2_collector[situation] = 0

                        if mean_dSOG1_pre > TOL:
                            SOG_alt_overshoot1_collector[situation] = np.max(
                                                dSOG1s_int / mean_dSOG1_pre)
                        else:
                            SOG_alt_overshoot1_collector[situation] = 0

                        if mean_dSOG2_pre > TOL:
                            SOG_alt_overshoot2_collector[situation] = np.max(
                                                dSOG2s_int / mean_dSOG2_pre)
                        else:
                            SOG_alt_overshoot2_collector[situation] = 0
                    else:
                        COG_alt_overshoot1_collector[situation] = 0
                        COG_alt_overshoot2_collector[situation] = 0
                        SOG_alt_overshoot1_collector[situation] = 0
    ...
    return (situation_numbers,
            ...
            steady_SOG1_change_collector,
            steady_SOG2_change_collector,
            steady_COG1_change_collector,
            steady_COG2_change_collector,
            #max_dSOG1_collector,
            #max_dSOG2_collector,
            #max_dCOG1_collector,
            #max_dCOG2_collector,
            ...
            COG_overshoot1_collector,
            COG_overshoot2_collector,
            SOG_overshoot1_collector,
            SOG_overshoot2_collector,
            COG_alt_maneuver1_collector,
            COG_alt_maneuver2_collector,
            SOG_alt_maneuver1_collector,
            SOG_alt_maneuver2_collector,
            COG_alt_overshoot1_collector,
            COG_alt_overshoot2_collector,
            SOG_alt_overshoot1_collector,
            SOG_alt_overshoot2_collector,
            ID_collector1,
            ID_collector2,
            Cat1_collector1,
            Cat1_collector2
